import type { FileDefinition, TypescriptDefinition, TypescriptReference } from './definition'
...
export function requireReference(
    ...
    if (!found) {
        ...
        const reference = typeof ref.reference === 'function' ? ref.reference() : ref.reference
        const name = `${ref.name}Local`
        const tsDefinition = toTypescriptDefinition(name, reference, false, locals)
        ...
        const extra = tsDefinition.references.map((ref) => requireReference(definitions, current, ref, locals)).flat()
        return [
            ...extra,
            {
                file: current.file,
...
export function scanModule(entry: string, definitions: { [k: string]: FileDefinition }, fullPath: string): void {
    ...
    for (const [name, symbol] of Object.entries(module)) {
        if (isExportable(symbol)) {
            ...
            const jsonschema = toJsonSchema(simplified)
            const tsDefinition = toTypescriptDefinition(name, simplified)
            ...
            //const foundDef = def?.[name]
            const typeOnly = false ///!(isShorthand(foundDef)) && foundDef[schema.uuid] === tsDefinition.uuid)
            definition.symbols.push({
            ...
            if (tsDefinition.meta !== undefined && !typeOnly) {
                const file = path.join(path.dirname(entry), schemaFile)
                definition.jsonFiles.push({ file, schema: JSON.stringify(jsonschema.schema, null, 2) })
            }
        ...
        schemaFiles.push(
            {
                ...
                template: ([] as string[])
                    .concat(
                        ...
                        ['/* eslint-disable */'],
                        definition.symbols
                            .filter((i) => i.tsDefinition?.meta !== undefined && !i.typeOnly && i.schemaFile)
                        ...
                        Object.values(definition.symbols)
                            .map((i) => [
                                ...
                                i.tsDefinition.meta !== undefined && !i.typeOnly
