...
import { $array, $boolean, $dict, $enum, $number, $object, $optional, $ref, $string, $unknown } from '~/index'
...
export const version_string = $string({ pattern: /^(?:\d{1,5}\.){0,3}\d{1,5}$/ })

export const page = $ref({ uri })

export const scripts = $array($ref({ uri }), {
    minItems: 1,
    uniqueItems: true,
})
...
export const glob_pattern = $string({
    //format: 'glob-pattern'
})

export const match_pattern = $string({
    //format: 'match-pattern'
    //pattern: /^((\\*|http|https|file|ftp|chrome-extension):\\/\\/(\\*|\\*\\.[^\\/\\*]+|[^\\/\\*]+)?(\\/.*))|<all_urls>$/
})
...
export const extension = $object({
    [$object.title]: 'JSON schema for Google Chrome extension manifest files',
    ...
    version: $ref({
        [$ref.description]: 'One to four dot-separated integers identifying the version of this extension.',
        version_string,
    }),
    ...
    icons: $object({
        ...
        '16': $ref({ [$ref.description]: "Used as the favicon for an extension's pages and infobar.", icon }),
        '48': $ref({ [$ref.description]: 'Used on the extension management page (chrome://extensions).', icon }),
        '128': $ref({ [$ref.description]: 'Used during installation and in the Chrome Web Store.', icon }),
        '256': $ref({ [$ref.description]: 'Used during installation and in the Chrome Web Store.', icon }),
    }),
    browser_action: $ref({
        [$ref.description]:
            'Use browser actions to put icons in the main Google Chrome toolbar, to the right of the address bar. In addition to its icon, a browser action can also have a tooltip, a badge, and a popup.',
    ...
    background: $object({
        ...
        page: $ref({
            [$ref.description]: 'Specify the HTML of the background page.',
            [$ref.default]: 'background.html',
            uri,
        }),
        scripts: $ref({
            [$ref.description]:
                'A background page will be generated by the extension system that includes each of the files listed in the scripts property.',
            [$ref.default]: ['background.js'],
            scripts,
        }),
    }),
    chrome_url_overrides: $object({
        [$object.description]:
            'Override pages are a way to substitute an HTML file from your extension for a page that Google Chrome normally provides.',
        [$object.maxProperties]: 1,

        bookmarks: $ref({
            [$ref.description]:
                'The page that appears when the user chooses the Bookmark Manager menu item from the Chrome menu or, on Mac, the Bookmark Manager item from the Bookmarks menu. You can also get to this page by entering the URL chrome://bookmarks.',
            [$ref.default]: 'bookmarks.html',
            page,
        }),
        history: $ref({
            [$ref.description]:
                'The page that appears when the user chooses the History menu item from the Chrome menu or, on Mac, the Show Full History item from the History menu. You can also get to this page by entering the URL chrome://history.',
            [$ref.default]: 'history.html',
            page,
        }),
        newtab: $ref({
            [$ref.description]:
                'The page that appears when the user creates a new tab or window. You can also get to this page by entering the URL chrome://newtab.',
            [$ref.default]: 'newtab.html',
            page,
        }),
    }),
    ...
    homepage_url: $ref({
        [$ref.description]: 'The URL of the homepage for this extension.',
        uri,
    }),
    ...
    minimum_chrome_version: $ref({
        [$ref.description]: 'The version of Chrome that your extension, app, or theme requires, if any.',
        version_string,
    }),
    nacl_modules: $array(
        $object({
            path: $ref({
                [$ref.description]: 'The location of a Native Client manifest (a .nmf file) within the extension directory.',
                uri,
            }),
            mime_type: $ref({
                [$ref.description]: 'The MIME type for which the Native Client module will be registered as content handler.',
                mime_type,
            }),
        }),
        {
    ...
    requirements: $object({
        ...
        '3D': $object({
            [$object.description]: "The '3D' requirement denotes GPU hardware acceleration.",
    ...
    sandbox: $object({
        ...
        content_security_policy: $ref({
            [$ref.default]: 'sandbox allow-scripts allow-forms',
            content_security_policy,
        }),
    }),
    short_name: $string({
    ...
    tts_engine: $object({
        [$object.description]: 'Register itself as a speech engine.',
        voices: $array(
            $object({
                //required: ['voice_name', 'event_types'],
                voice_name: $string({
